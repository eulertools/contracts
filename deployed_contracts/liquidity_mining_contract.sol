...
contract EulerLiquidityMining is Ownable {
    using SafeMath for uint256;
    ...
    IERC20 public euler;
    uint256 public eulerPerBlock = uint256(40 ether); //40 euler
    ...
    function setEulerTokens(IERC20 _euler, IERC20 _lpToken) external onlyOwner {
        require(address(euler) == address(0) && address(liquidityMining.lpToken) == address(0), 'Tokens already set!');
        euler = _euler;
        liquidityMining =
            PoolInfo({
                lpToken: _lpToken,
    ...
    function pendingRewards(address _user) external view returns (uint256) {
        ...
        if (block.number > liquidityMining.lastRewardBlock && lpSupply != 0) {
            ...
            accPerShare = liquidityMining.accPerShare.add(eulerReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool() internal {
        ...
        uint256 multiplier = block.number.sub(liquidityMining.lastRewardBlock);
        uint256 eulerReward = multiplier.mul(eulerPerBlock);
        liquidityMining.accPerShare = liquidityMining.accPerShare.add(eulerReward.mul(1e12).div(lpSupply));
        liquidityMining.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 amount) external {
        ...
        updatePool();
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(liquidityMining.accPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(liquidityMining.accPerShare).div(1e12);
    ...
    function claim() external {
        ...
        if (pending > 0 || user.pendingRewards > 0) {
            user.pendingRewards = user.pendingRewards.add(pending);
            uint256 claimedAmount = safeEulerTransfer(msg.sender, user.pendingRewards);
    ...
    function safeEulerTransfer(address to, uint256 amount) internal returns (uint256) {
        uint256 eulerBalance = euler.balanceOf(address(this));
        if (amount > eulerBalance) {
            euler.safeTransfer(to, eulerBalance);
            return eulerBalance;
        } else {
